...
def split_train_test(file_name, label):
    training_set_file = 'D:/Code/PSP/data/training_set.csv'
    test_set_file = 'D:/Code/PSP/data/test_set.csv'
...
def main():
    ...
    while count < 36:
        if mode == 1:
            mode = 2
            filename = f'D:/Code/PSP/data/csv/Subject{str(count).zfill(2)}_2.csv'
            label = [1]
            count += 1
        elif mode == 2:
            mode = 1
            filename = f'D:/Code/PSP/data/csv/Subject{str(count).zfill(2)}_1.csv'
            label = [0]
